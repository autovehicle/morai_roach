# 벡터화된 환경 구조 요구 문제
# main.py 짤 때, MORAI UDP 환경 그냥 넣지 말고, stable_baselines3에서 제공하는 DummyVecEnv로 한번 감싸서 넣어주기
...
class PPO():
    def __init__(self, policy, env,
                 ...
                 n_steps_total: int = 8192, # 수집 시 시간 걸릴 수 있기 때문에 수정 필요할 수 있음.
                 batch_size: int = 256,
                 n_epochs: int = 20,
                 gamma: float = 0.99,
    ...
    def collect_rollouts(self, env: VecEnv, callback: BaseCallback,
        ...
        while n_steps < n_rollout_steps:
            ...
            # 확인: MORAI Wrapper에서 episode_stat라는 키 만들어줘야함
            # update_info_buffer
            # MORAI 환경의 step() 반환값인 info[i]에 episode_stat이라는 키가 반드시 존재헤야 함.
            # 만약 없다면 무조건 에러 발생하므로, 안전하게 처리하기
            for i in np.where(dones)[0]:
                if 'episode_stat' in infos[i]:
    ...
    def learn(self, total_timesteps, callback=None, seed=2021):
        # reset env seed
        # MORAI env 클래스 내부에 seed(self, seed=None) 구현되어있어야 함. 만약 없다면, env.seed(seed) 호출 시 에러 발생
