# 바꿔야 하는 부분
# 1. CARLA 전용 라이브러리 Import문 지우기
# 2. __init__ 함수 내부에서 클래스를 동적으로 불러오는 부분을 직접 Import하는 방식으로 바꾸기
...
from rl_birdview.models.distributions import BetaDistribution
from rl_birdview.models.torch_layers import BirdviewExtractor
...
class PpoPolicy(nn.Module):
    ...
    def __init__(self,
        ...
        self.features_extractor = BirdviewExtractor(observation_space, **features_extractor_kwargs)

        # BetaDistribution 사용(action_dim은 action_space의 shape에 맞게 조정 필요)
        action_dim = int(np.prod(action_space.shape))
        self.action_dist = BetaDistribution(action_dim, **distribution_kwargs)
