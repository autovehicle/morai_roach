# 바꿔야 하는 부분
# 1. info 딕셔너리 구조를 MORAI 환경에 맞게 간소화하거나, MORAI 통신환경에서 똑같은 구조의 딕셔너리 만들기
# 2. 시각화 함수 인덱스: vis_idx 부분 MORAI에서 생성할 BEV 채널 순서에 맞게 번호 수정하기
...
class PpoBuffer():
    ...
    def add(self,
        ...
        # MORAI 환경에 맞는 Infor 딕셔너리 구조에 맞게 수정해야함
        for i in range(self.n_envs):
            # MORAI 환경에서 reward_debug, 정보를 안 보낼 수 있기 때문에 get() 함수로 안전하게 가져오기
            reward_info = infos[i].get('reward_debug', {'debug_texts': []})
            self.reward_debugs[i].append(reward_info['debug_texts'])

            terminal_info = infos[i].get('terminal_debug', {
                'debug_texts': [],
                'exploration_suggest': {'n_steps': 0, 'suggest': None}
            })
            self.terminal_debugs[i].append(terminal_info['debug_texts'])

            n_steps = terminal_info['exploration_suggest']['n_steps']
            if n_steps > 0:
                n_start = max(0, self.pos - n_steps)
                self.exploration_suggests[n_start:self.pos, i] = \
                    terminal_info['exploration_suggest']['suggest']
    ...
    # MORAI BEV 렌더러의 채널 설계에 맞게 수정
    def render(self):
        assert self.full, ''
        list_render = []

        _, _, c, h, w = self.observations['birdview'].shape
        vis_idx = np.array([0, 1, 2, 6, 10, 14]) # 이 부분 수정 필요함
