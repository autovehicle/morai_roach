...
def main(cfg: DictConfig):
    # CARLA 서버 강제 종료 및 실행하는 부분: MORAI에서는 사용 안하니까 삭제
    # if cfg.kill_running:
    ...
    # CARLA 전용 맵 파일 체크하는 부분: MORAI 시뮬레이터 사용하니까 그거에 맞게 변경하기 위해 주석처리
    # config_utils.check_h5_maps(cfg.train_envs, obs_configs, cfg.carla_sh_path)

    # MORAI UDP 통신 환경 불러오도록 변경하기
    ...
    # 다중 환경을 단일 환경으로 고정: MORAI는 기본적으로 1개의 시뮬레이터와 통신하므로 DummyVecEnv 하나로 고정
    ...
    # CARLA 서버 종료 부분: MORAI 시뮬레이터 사용하니까 필요없어서 주석처리
    # server_manager.stop()
